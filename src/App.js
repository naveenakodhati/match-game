...
class App extends Component {
  state = {
    ...
    displayLists: imagesList.filter(eachTab => eachTab.category === 'FRUIT'),
    score: 0,
    isDisplay: true,
    timer: 60,
  }
  ...
  onRandomImage = () => {
    const randomTab = Math.ceil(Math.random() * tabsList.length) - 1
    const getImages = imagesList.filter(
      eachList => eachList.category === tabsList[randomTab].tabId,
    )
    const randomImage = Math.ceil(Math.random() * getImages.length) - 1
    const getRandomImage = getImages[randomImage]
    this.setState({imageObj: getRandomImage})
  }
  ...
  onSetTabItem = id => {
    ...
    this.setState({tabCategory: id, displayLists: filterImages})
  }
  ...
  playAgain = () => {
    this.setState({timer: 60, isDisplay: true, score: 0}, this.setTimer)
  }
      ...
      <div className="main-app-container">
          ...
          <ul className="nav-display">
            ...
            <li className="nav-display">
              <img
                ...
                alt="timer"
              />
              <p className="nav-score">{timer} sec</p>
            </li>
          </ul>
        </nav>
        ...
        {isDisplay ? (
          <div className="cards-container">
            ...
            <ul className="tab-ul-container">
              {tabsList.map(eachTab => (
                <TabItems
                  ...
                  onSetTabItem={this.onSetTabItem}
                  key={eachTab.tabId}
                  tabData={eachTab}
                />
              ))}
            ...
            <ul className="tab-ul-container">
              {displayLists.map(eachImageItem => (
                <ThumbnailsItem
                  onImageClicked={this.onImageClicked}
                  key={eachImageItem.id}
                  thumbnailsData={eachImageItem}
                />
              ))}
            </ul>
          </div>
